"""CLI — the local demo entrypoint. Runs with zero external services.
    ...
    python -m grounded review <diff-file> [--mode baseline|llm|specialists]
    python -m grounded eval [--report] [--min-precision F]
    python -m grounded trace <review_id>            (M4)
"""
...
from grounded.evaluation import evaluate
from grounded.hitl import ApprovalQueue, TicketStatus
from grounded.models import Decision
...
def cmd_review(args) -> int:
    ...
    if args.emit_events:
        ...
        print(f"[events -> {args.emit_events}]  (reconstruct with: grounded trace {review.review_id} --events {args.emit_events})")
...
def cmd_serve(args) -> int:
    ...
    print(f"grounded ingress on http://{args.host}:{args.port}  (POST /webhook/github, GET /healthz)")
    uvicorn.run("grounded.api.app:app", host=args.host, port=args.port, reload=args.reload)
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="grounded", description="Selective, grounded, failure-aware PR reviewer.")
